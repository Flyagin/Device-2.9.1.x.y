...
30(с)х50(періодів)х16(виборок на період)х(8(канали) + 15(двобайтні слова дискретних сигналів))
х2(байти на число) = 1104000(байти)
...
Тобто розмір максимального аналогового запису 1104000+60 = 1104060(байти)
З урахуванням того, що розмір сторінки DF становить 512 байт, то максимальний
аналоговий запис становить 1104060/512 = 2156.3671875 сторінки. Значить буде вестися 
запис у 2157 сторінки DF.
...
Тоді для запису 2157 сторінок треба буде
2157(сторінки)х51(мс) = 110 007(мс)=119,007(с)

Тобто процес запису максимального аналогового запису буде тривати 110,007(с)

Швидкість "заповнення" миттєвими виборками і дискретними сигналами з вимірювальної системи:
(8(канали) + 15(двобайтні слова дискретних сигналів))х2(байти на значення)/1,25(мс
- інтервал між виборками при частоті 50Гц і 16 виборок на період) =36,8 (байт/мс)
...
2157(сторінки)х512(байт на сторінку)/110007(мс) = 10.039215686274509803921568627451 (байт/мс)
...
буде виконуватися зі швидкістю: 36,8 - 10.039215686274509803921568627451 = 26.760784313725490196078431372549(байт/мс)
...
25000(мс)*26.760784313725490196078431372549(байт/мс) = 669019.60784313725490196078431373(байт) < 669020(байт)
669020(байт)/512(байт/стор.) = 1306.6796875(сторінок) < 1307(сторінок)
...
(1307 + 1) сторінки DF). Ще одну сторінку до 1308 теоретично розрахованих я додав 
для перестаховки.

Тобто розмір має бути
5(с - макс. доавар. мас.)х50(пром.частота)х16(кільк. вибор.за пер. для ан.р.)х(8(канали) + 18(двобайтні слова дискретних сигналів)) +1307*[512(байт)/2(байт на одне миттєве значення)](кількість миттєвих значень на сторінці)=
= 92000 +996х256 = 426592
...
1307(сторінок)х256(слів) = 3345926 (слів)

334592/(8 + 15)= 14547.478260869565217391304347826
Якщо ділення не є ціле число, то треба провести корекцію, щоб Х/(8 + 15)
давало ціле число, причому Х > 3345926 - тобто беремо найслижче число X = 334604

таким чином
92000 + 3345926 = 426604

!!!Вищевикладена дає мені підставу ширину масиву встановити 426604 елементи (двохбайтені,
так як кожна виборка складається із двох байт)!!!

*/

#define SIZE_BUFFER_FOR_AR    131062/*426604*/
