...
void make_ekran_energy(unsigned int pervynna_vtorynna)
{
  ...
  {
    if (index_of_ekran < (MAX_ROW_FOR_EKRAN_ENERGY<<1))//ћноженн€ на два константи MAX_ROW_FOR_EKRAN_ENERGY потр≥бне дл€ того, бо на одну позиц≥ю ми використовуЇмо два р€дки (назва + значенн€)
    {
      if ((i & 0x1) == 0)
      {
